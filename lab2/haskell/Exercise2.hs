...
fillStatesOut (state:[]) li lu s n = return []
fillStatesOut (state:xs) li lu s n = do
    reaction <- elements (li ++ lu)
    rest <- fillStatesOut xs li lu s n
    return ((state, reaction, (state+1)) : rest)

genIOLTSOut :: Gen IOLTS
genIOLTSOut = do
    n <- choose (0, 10)
    let q = [0..n]
    (li, lu) <- genLabels
