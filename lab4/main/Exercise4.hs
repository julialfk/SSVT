...
-- Any non-empty domain under an empty relation, should return false because this is never serial.
-- Input for this function should come from 'genNonEmptyDomain'
prop_EmptyList :: Eq a => [a] -> Property
prop_EmptyList domain = property ((isSerial domain []) == False)

-- Reflexive relations, by nature, are always serial.
-- Input for this function should come from 'genReflexiveDomainsAndRelations'
