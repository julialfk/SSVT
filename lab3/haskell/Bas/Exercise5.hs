...
mutateEquivalence :: (([Integer] -> Integer -> Bool), ([Integer] -> Integer -> Bool)) -> (Integer -> [Integer]) -> Gen Bool
mutateEquivalence (prop1, prop2) f = do
    input <- choose(0,100)
...
generatorsEquivalence :: [([Integer] -> Integer -> Bool)] -> (Integer -> [Integer]) -> [Gen Bool]
generatorsEquivalence props f = [mutateEquivalence (prop1, prop2) f | prop1 <- props, prop2 <- props]

main :: IO ()
main = do
  putStrLn "Testing QuickCheck generators:"
  mapM_ quickCheck generatorsEquivalence