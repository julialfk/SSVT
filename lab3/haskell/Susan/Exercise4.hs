...
propertySetStrength :: Integer -> [([Integer] -> Integer -> Bool)] -> (Integer -> [Integer]) -> Float
propertySetStrength totalNumberOfMutants listOfProperties functionUnderTest =
    fromIntegral (totalNumberOfMutants - (Ex2.countSurvivors totalNumberOfMutants listOfProperties functionUnderTest)) / fromIntegral totalNumberOfMutants
